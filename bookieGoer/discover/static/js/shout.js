...
function shout_init() {
  $("#shout").submit(function() {
    ...
    $.post("/api/shouts/new", { lat: lat, lng: lng, author: author.val(), message: message.val(), keywords: keywords.val() }, function(data) {
      var new_shouts = $.parseJSON(data);
      for (var i=0; i<new_shouts.length; i++) {
        //alert('i = ' + i);
        add_shout(new_shouts[i]);
      }
